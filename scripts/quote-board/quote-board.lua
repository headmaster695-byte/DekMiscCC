...
local SONGS = {
  ...
  -- ── Title track ────────────────────────────────────────────────────────────
  -- "A New Start" by Nanashi Mumei — Lofi Minecraft-style remix.
  -- Original: G major, 120 BPM, Celtic folk, 3:49 (Tomoari Taguma / Mori Calliope).
  -- This arrangement slows the feel to ~90 BPM and strips it back to harp and
  -- chime, in the spirit of C418's Minecraft ambient pieces.
  -- Structure: Intro → V1a → V1b → Pre-C → Chorus → Interlude → V2 →
  --            Pre-C → Chorus 2 → Bridge → Final Chorus → Outro  (~2:40)
  -- Pitch ref (G major): G3=1 B3=5 C4=6 D4=8 E4=10 F#4=12 G4=13 A4=15
  --                      B4=17 C5=18 D5=20 E5=22 F#5=24
  -- This song always plays first on startup; all subsequent picks are random.
  {
    name  = "A New Start (Lofi Remix)",
    notes = {
      -- ── INTRO ──────────────────────────────────────────────────────────────
      -- Celtic harp arpeggios, G – Em – C – D (two full passes).
      -- Pass 1: hushed, no melody yet — just the harmonic atmosphere settling in.
      {H, 0.7,  1, 0.30}, {H, 0.6,  8, 0.25}, {H, 0.6, 13, 0.25}, {H, 0.6, 17, 0.85},
      {H, 0.7, 10, 0.30}, {H, 0.6, 13, 0.25}, {H, 0.6, 17, 0.25}, {H, 0.6, 22, 0.85},
      {H, 0.7,  6, 0.30}, {H, 0.6, 10, 0.25}, {H, 0.6, 13, 0.25}, {H, 0.6, 18, 0.85},
      {H, 0.7,  8, 0.30}, {H, 0.6, 12, 0.25}, {H, 0.6, 15, 0.25}, {H, 0.6, 20, 1.30},
      -- Pass 2: slightly fuller; chime echoes the peak of each chord.
      {H, 0.8,  1, 0.30}, {H, 0.7,  8, 0.25}, {H, 0.7, 13, 0.25}, {H, 0.7, 17, 0.25}, {CH, 0.5, 17, 0.60},
      {H, 0.8, 10, 0.30}, {H, 0.7, 13, 0.25}, {H, 0.7, 17, 0.25}, {H, 0.7, 22, 0.25}, {CH, 0.5, 22, 0.60},
      {H, 0.8,  6, 0.30}, {H, 0.7, 10, 0.25}, {H, 0.7, 13, 0.25}, {H, 0.7, 18, 0.25}, {CH, 0.5, 18, 0.60},
      {H, 0.8,  8, 0.30}, {H, 0.7, 12, 0.25}, {H, 0.7, 15, 0.25}, {H, 0.7, 20, 0.25}, {CH, 0.5, 20, 1.80},
      -- ── VERSE 1A ───────────────────────────────────────────────────────────
      -- Main melody enters — stepwise G-major ascent, the Celtic folk fingerprint.
      -- G4 A4 B4  —  D5 B4 A4 G4  (bars 1–2, G chord)
      {H, 1.0, 13, 0.55}, {H, 0.9, 15, 0.40}, {H, 1.0, 17, 0.55},
      {H, 1.0, 20, 0.40}, {H, 0.9, 17, 0.30}, {H, 0.9, 15, 0.30}, {H, 1.0, 13, 1.10},
      -- E4 G4 A4  —  B4 D5 B4  (bars 3–4, Em chord)
      {H, 1.0, 10, 0.55}, {H, 0.9, 13, 0.40}, {H, 0.9, 15, 0.40},
      {H, 1.0, 17, 0.55}, {H, 1.0, 20, 0.40}, {H, 0.9, 17, 1.10},
      -- C5 B4 A4  —  G4 E4 G4  (bars 5–6, C chord)
      {H, 1.0, 18, 0.55}, {H, 0.9, 17, 0.35}, {H, 0.9, 15, 0.35},
      {H, 0.9, 13, 0.40}, {H, 0.9, 10, 0.35}, {H, 0.9, 13, 0.95},
      -- D5 B4 A4  —  G4 long  (bars 7–8, D chord, breath before repeat)
      {H, 1.0, 20, 0.55}, {H, 0.9, 17, 0.35}, {H, 0.9, 15, 0.35},
      {H, 0.9, 13, 1.90},
      -- ── VERSE 1B ───────────────────────────────────────────────────────────
      -- Second phrase: reaches up to E5, adds F#4 (Celtic Mixolydian flicker).
      -- G4 A4 B4 D5  —  E5 D5 B4  (bars 9–10)
      {H, 1.0, 13, 0.50}, {H, 0.9, 15, 0.35}, {H, 0.9, 17, 0.35},
      {H, 1.0, 20, 0.50}, {H, 1.1, 22, 0.40}, {H, 1.0, 20, 0.35}, {H, 0.9, 17, 1.00},
      -- A4 G4 F#4  —  G4 A4 B4  (bars 11–12, leading tone adds the Celtic bite)
      {H, 0.9, 15, 0.45}, {H, 0.9, 13, 0.35}, {H, 0.8, 12, 0.35},
      {H, 0.9, 13, 0.45}, {H, 0.9, 15, 0.35}, {H, 1.0, 17, 1.10},
      -- D5 B4 G4  —  E4 G4 long  (bars 13–14, settle back down)
      {H, 1.0, 20, 0.50}, {H, 0.9, 17, 0.35}, {H, 0.9, 13, 0.40},
      {H, 0.9, 10, 0.40}, {H, 0.9, 13, 1.60},
      -- ── PRE-CHORUS 1 ───────────────────────────────────────────────────────
      -- Tension builds; walks up through Em – Am/C – D.
      {H, 1.0, 10, 0.45}, {H, 1.0, 13, 0.35}, {H, 1.0, 15, 0.35}, {H, 1.1, 17, 0.65},
      {H, 1.0, 15, 0.30}, {H, 1.0, 13, 0.30}, {H, 1.0, 10, 0.85},
      {H, 1.0,  6, 0.45}, {H, 1.0, 10, 0.35}, {H, 1.0, 13, 0.35}, {H, 1.1, 15, 0.65},
      {H, 1.0, 17, 0.35}, {H, 1.0, 15, 0.35}, {H, 1.2, 20, 1.40},
      -- ── CHORUS 1 ───────────────────────────────────────────────────────────
      -- The emotional peak — high register, broader leaps, chime shimmers in.
      {H, 1.2, 17, 0.55}, {H, 1.1, 20, 0.40}, {H, 1.2, 22, 0.55},
      {H, 1.0, 20, 0.40}, {H, 1.0, 17, 0.35}, {H, 0.9, 15, 1.00},
      {H, 1.2, 13, 0.55}, {H, 1.1, 17, 0.40}, {H, 1.2, 20, 0.55},
      {H, 1.1, 22, 0.40}, {H, 1.0, 20, 0.30}, {H, 0.9, 17, 1.10},
      {H, 1.1, 18, 0.55}, {H, 1.0, 17, 0.35}, {H, 1.0, 15, 0.35},
      {H, 0.9, 13, 0.45}, {H, 0.9, 15, 0.30}, {H, 0.9, 17, 0.75}, {CH, 0.6, 22, 0.35},
      {H, 1.0, 15, 0.45}, {H, 0.9, 13, 0.35}, {H, 0.9, 10, 0.35},
      {H, 0.9, 13, 1.80},
      -- ── INTERLUDE ──────────────────────────────────────────────────────────
      -- Arpeggios return alone — a moment of stillness, like turning a page.
      {CH, 0.6,  1, 0.30}, {CH, 0.5,  8, 0.25}, {CH, 0.5, 13, 0.25}, {CH, 0.5, 17, 0.85},
      {CH, 0.6, 10, 0.30}, {CH, 0.5, 13, 0.25}, {CH, 0.5, 17, 0.25}, {CH, 0.5, 22, 0.85},
      {CH, 0.6,  6, 0.30}, {CH, 0.5, 10, 0.25}, {CH, 0.5, 13, 0.25}, {CH, 0.5, 18, 0.85},
      {CH, 0.6,  8, 0.30}, {CH, 0.5, 12, 0.25}, {CH, 0.5, 15, 0.25}, {CH, 0.5, 20, 1.90},
      -- ── VERSE 2 ────────────────────────────────────────────────────────────
      -- Quieter and more introspective; chime echoes chase each phrase.
      {H, 0.9, 13, 0.55}, {H, 0.8, 15, 0.40}, {H, 0.9, 17, 0.55},
      {H, 0.9, 20, 0.40}, {H, 0.8, 17, 0.30}, {H, 0.8, 15, 0.30}, {H, 0.9, 13, 0.70},
      {CH, 0.4, 13, 0.50},
      {H, 0.9, 10, 0.55}, {H, 0.8, 13, 0.40}, {H, 0.8, 15, 0.40},
      {H, 0.9, 17, 0.55}, {H, 0.9, 20, 0.40}, {H, 0.8, 17, 0.70},
      {CH, 0.4, 17, 0.50},
      {H, 0.9, 18, 0.55}, {H, 0.8, 17, 0.35}, {H, 0.8, 15, 0.35},
      {H, 0.8, 13, 0.45}, {H, 0.8, 10, 0.35}, {H, 0.8, 13, 0.85},
      {H, 0.9, 20, 0.50}, {H, 0.9, 17, 0.40}, {H, 0.9, 15, 0.35},
      {H, 0.9, 13, 1.80},
      -- ── PRE-CHORUS 2 ───────────────────────────────────────────────────────
      {H, 1.0, 10, 0.45}, {H, 1.0, 13, 0.35}, {H, 1.0, 15, 0.35}, {H, 1.1, 17, 0.65},
      {H, 1.0, 15, 0.30}, {H, 1.0, 13, 0.30}, {H, 1.0, 10, 0.75}, {CH, 0.5, 10, 0.40},
      {H, 1.0,  6, 0.45}, {H, 1.0, 10, 0.35}, {H, 1.0, 13, 0.35}, {H, 1.1, 15, 0.65},
      {H, 1.1, 17, 0.35}, {H, 1.1, 20, 0.35}, {H, 1.2, 22, 1.60},
      -- ── CHORUS 2 ───────────────────────────────────────────────────────────
      -- Fuller and more assured than the first; add a chime halo on the peaks.
      {H, 1.2, 17, 0.55}, {H, 1.1, 20, 0.40}, {H, 1.2, 22, 0.55},
      {H, 1.1, 20, 0.40}, {H, 1.1, 22, 0.30}, {H, 1.1, 20, 0.80}, {CH, 0.7, 22, 0.35},
      {H, 1.2, 13, 0.55}, {H, 1.1, 17, 0.40}, {H, 1.2, 20, 0.55},
      {H, 1.1, 22, 0.40}, {H, 1.0, 20, 0.30}, {H, 1.0, 17, 1.00},
      {H, 1.1, 18, 0.55}, {H, 1.0, 17, 0.35}, {H, 1.0, 15, 0.35},
      {H, 1.0, 13, 0.45}, {H, 1.0, 15, 0.30}, {H, 1.0, 17, 0.70}, {CH, 0.6, 17, 0.35},
      {H, 1.0, 20, 0.50}, {H, 1.0, 17, 0.40}, {H, 1.0, 15, 0.35},
      {H, 1.1, 13, 1.90},
      -- ── BRIDGE ─────────────────────────────────────────────────────────────
      -- The most melancholic passage — drops to the lower Em–C territory.
      -- Each note breathes longer here; the tempo seems to slow further.
      {H, 0.8, 10, 0.70}, {H, 0.7, 13, 0.60}, {H, 0.7, 17, 0.60},
      {H, 0.7, 13, 0.50}, {H, 0.7, 10, 0.50}, {H, 0.8,  8, 1.20},
      {H, 0.8,  6, 0.70}, {H, 0.7, 10, 0.60}, {H, 0.7, 13, 0.60},
      {H, 0.7, 15, 0.50}, {H, 0.7, 13, 0.50}, {H, 0.8, 10, 1.20},
      -- Second bridge phrase lifts tentatively toward D5 before falling back
      {H, 0.8,  8, 0.70}, {H, 0.7, 12, 0.55}, {H, 0.7, 15, 0.55},
      {H, 0.8, 17, 0.55}, {H, 0.9, 20, 0.45}, {H, 0.8, 17, 0.80},
      {H, 0.9, 15, 0.60}, {H, 0.8, 13, 0.50}, {H, 0.8, 10, 0.50},
      {H, 0.9, 13, 2.00},
      -- ── FINAL CHORUS ───────────────────────────────────────────────────────
      -- The emotional apex — full volume, reaches F#5 (the leading tone),
      -- then resolves to G for the last time.
      {H, 1.3, 17, 0.55}, {H, 1.2, 20, 0.40}, {H, 1.3, 22, 0.55},
      {H, 1.2, 20, 0.40}, {H, 1.2, 22, 0.30}, {H, 1.3, 24, 0.55}, {CH, 0.8, 24, 0.35},
      {H, 1.3, 22, 0.55}, {H, 1.2, 20, 0.40}, {H, 1.2, 17, 0.55},
      {H, 1.1, 22, 0.40}, {H, 1.1, 20, 0.30}, {H, 1.0, 17, 1.00},
      {H, 1.2, 18, 0.55}, {H, 1.1, 17, 0.35}, {H, 1.1, 15, 0.35},
      {H, 1.0, 13, 0.45}, {H, 1.0, 15, 0.30}, {H, 1.1, 17, 0.70}, {CH, 0.7, 22, 0.35},
      {H, 1.2, 20, 0.55}, {H, 1.1, 17, 0.40}, {H, 1.1, 15, 0.35},
      {H, 1.2, 13, 1.30},
      -- ── OUTRO ──────────────────────────────────────────────────────────────
      -- The arpeggios return and grow quieter with each pass — like the music
      -- fading into the distance as you walk away into a new world.
      {H, 0.8,  1, 0.30}, {H, 0.7,  8, 0.25}, {H, 0.7, 13, 0.25}, {H, 0.7, 17, 0.85},
      {H, 0.8, 10, 0.30}, {H, 0.7, 13, 0.25}, {H, 0.7, 17, 0.25}, {H, 0.7, 22, 0.85},
      {H, 0.7,  6, 0.30}, {H, 0.6, 10, 0.25}, {H, 0.6, 13, 0.25}, {H, 0.6, 18, 0.85},
      {H, 0.7,  8, 0.30}, {H, 0.6, 12, 0.25}, {H, 0.6, 15, 0.25}, {H, 0.6, 20, 1.10},
      {H, 0.6,  1, 0.35}, {H, 0.5,  8, 0.30}, {H, 0.5, 13, 0.30}, {H, 0.5, 17, 1.10},
      {CH, 0.4, 17, 0.50},
      {H, 0.5,  1, 0.40}, {H, 0.4,  8, 0.35}, {H, 0.4, 13, 0.35}, {H, 0.4, 17, 1.30},
      {CH, 0.3, 13, 0.60},
      {H, 0.4,  1, 0.50}, {H, 0.3,  8, 0.40}, {H, 0.3, 13, 0.40},
      {H, 0.3, 17, 3.00},
    },
  },
