...
local function musicLoop(speakers)
  ...
  while true do
    ...
    -- Wrap in pcall so a bad note in one song cannot permanently silence the loop
    local ok, err = pcall(playSong, song, speakers)
    if not ok then
      state.songName = "err"   -- brief visual cue; next song will overwrite
      os.sleep(0.5)            -- yield so other coroutines stay responsive
    end
  end
end
